...
class TestGipHashObject(unittest.TestCase):
    ...
    def check_index(self, filename: str) -> None:
        ...
        e = entries[0]
        self.assertEqual(statinfo.st_ctime, e.ctime_s)
        self.assertEqual(statinfo.st_ctime_ns, e.ctime_n)
        self.assertEqual(statinfo.st_mtime, e.mtime_s)
        self.assertEqual(statinfo.st_mtime_ns, e.mtime_n)
        ...
        self.assertEqual(sha1_hash, e.sha1)
